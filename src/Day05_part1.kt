fun main() {
    fun part1(input: List<String>): String {
        ...
        val containers = convertInput(input.subList(0, emptyLineIndex))
        val inputLineRegex = """move (\d+) from (\d+) to (\d+)""".toRegex()

        for (it in input.subList(emptyLineIndex + 1, input.size)) {
            ...
            containers[to].addAll(fromContainer.reversed())
        }
        var answer = ""
